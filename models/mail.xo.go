...
type Mail struct {
	...
	TsDeleted   sql.NullInt64 // ts_deleted
...
func (m *Mail) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO mail (` +
		`sender_id, recipient_id, content, ts_received, subject, is_deleted, ts_deleted` +
		`) VALUES (` +
		`?, ?, ?, ?, ?, ?, ?` +
		`)`

	// run query
	XOLog(sqlstr, m.SenderID, m.RecipientID, m.Content, m.TsReceived, m.Subject, m.IsDeleted, m.TsDeleted)
	res, err := db.Exec(sqlstr, m.SenderID, m.RecipientID, m.Content, m.TsReceived, m.Subject, m.IsDeleted, m.TsDeleted)
	if err != nil {
		return err
	}
...
func (m *Mail) Update(db XODB) error {
	...
	// run query
	XOLog(sqlstr, m.SenderID, m.RecipientID, m.Content, m.TsReceived, m.Subject, m.IsDeleted, m.TsDeleted, m.ID)
	_, err = db.Exec(sqlstr, m.SenderID, m.RecipientID, m.Content, m.TsReceived, m.Subject, m.IsDeleted, m.TsDeleted, m.ID)
	return err
}
...
func MailsByIsDeletedTsDeleted(db XODB, isDeleted int64, tsDeleted sql.NullInt64) ([]*Mail, error) {
	var err error
...
func MailByID(db XODB, id int64) (*Mail, error) {
	...
	err = db.QueryRow(sqlstr, id).Scan(&m.ID, &m.SenderID, &m.RecipientID, &m.Content, &m.TsReceived, &m.Subject, &m.IsDeleted, &m.TsDeleted)
	if err != nil {
		return nil, err
	}
