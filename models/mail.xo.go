...
type Mail struct {
	...
	TsReceived int    // ts_received
	Subject    string // subject
	IsDeleted  int    // is_deleted
	TsDeleted  int    // ts_deleted
...
func (m *Mail) Insert(db XODB) error {
	...
	const sqlstr = `INSERT INTO mail (` +
		`sender_id, content, ts_received, subject, is_deleted, ts_deleted` +
		`) VALUES (` +
		`?, ?, ?, ?, ?, ?` +
		`)`

	// run query
	XOLog(sqlstr, m.SenderID, m.Content, m.TsReceived, m.Subject, m.IsDeleted, m.TsDeleted)
	res, err := db.Exec(sqlstr, m.SenderID, m.Content, m.TsReceived, m.Subject, m.IsDeleted, m.TsDeleted)
	if err != nil {
		return err
	}
...
func (m *Mail) Update(db XODB) error {
	...
	// run query
	XOLog(sqlstr, m.SenderID, m.Content, m.TsReceived, m.Subject, m.IsDeleted, m.TsDeleted, m.ID)
	_, err = db.Exec(sqlstr, m.SenderID, m.Content, m.TsReceived, m.Subject, m.IsDeleted, m.TsDeleted, m.ID)
	return err
}
...
func MailsByIsDeletedTsDeleted(db XODB, isDeleted int, tsDeleted int) ([]*Mail, error) {
	var err error

	// sql query
	const sqlstr = `SELECT ` +
		`id, sender_id, content, ts_received, subject, is_deleted, ts_deleted ` +
		`FROM mail ` +
		`WHERE is_deleted = ? AND ts_deleted = ?`

	// run query
	XOLog(sqlstr, isDeleted, tsDeleted)
	q, err := db.Query(sqlstr, isDeleted, tsDeleted)
	if err != nil {
		return nil, err
	}
	defer q.Close()
	...
	for q.Next() {
		...
		// scan
		err = q.Scan(&m.ID, &m.SenderID, &m.Content, &m.TsReceived, &m.Subject, &m.IsDeleted, &m.TsDeleted)
		if err != nil {
			return nil, err
		}
...
func MailsBySenderID(db XODB, senderID int) ([]*Mail, error) {
	var err error
...
func MailsByTsReceived(db XODB, tsReceived int) ([]*Mail, error) {
	var err error
...
func MailByID(db XODB, id int) (*Mail, error) {
	var err error
	...
	err = db.QueryRow(sqlstr, id).Scan(&m.ID, &m.SenderID, &m.Content, &m.TsReceived, &m.Subject, &m.IsDeleted, &m.TsDeleted)
	if err != nil {
		return nil, err
	}
